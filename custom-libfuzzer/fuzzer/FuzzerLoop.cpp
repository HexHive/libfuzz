...
void StoreFeedbackAndExit(char cause) {
...
system_clock::time_point LastSeedFound;
system_clock::time_point FuzzingiStart;
...
void Fuzzer::MutateAndTestOne() {
  ...
  for (int i = 0; i < Options.MutateDepth; i++) {
    ...
    if (Fuzzer::MeasureCoveragePlateau) {
      if (isFirstSeed) {
        isFirstSeed = false;
        LastSeedFound = system_clock::now();
      }

      if (NewCov) {
        LastSeedFound = system_clock::now();
      } else {
        auto TimeFromLastSeedFound = duration_cast<seconds>(
          system_clock::now() - LastSeedFound).count();
        if (TimeFromLastSeedFound >= Fuzzer::CovPlateauTimeout) {
