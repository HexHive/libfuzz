...
class JavaContext:
    ...
    def get_random_var(self, type: JavaType) -> Variable:
        if type in self.variable_alive and random.random() > self.prob_gen:
            ...
            if type in self.ret_dict and random.randint(0, 1) == 0:
                ...
                if self.create_for_builtin(type):
                    var = Variable(type)
    ...
    # this function handles create builtin type. If the type is not a builtin type, it returns false. Otherwise it returns true
    def create_for_builtin(self, type: JavaType) -> bool:
