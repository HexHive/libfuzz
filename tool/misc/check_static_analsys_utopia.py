...
def get_utopia_data(utopia_folder, all_exposed_func):

    real_utipia_api = {}

    for p in os.listdir(utopia_folder):
        l = p.split(".")[0]

        real_utipia_api[l] = {}

        an_path = os.path.join(utopia_folder, p)
        with open(an_path) as f:
            ...
            for f, p_b in uar["Array"].items():
                f_clean = f.split("(")[0]
                # print(f_clean)
                if (f_clean in all_exposed_func[l] and 
                    "[" not in f and p_b != -1):
                    ...
                    arg_info = {}
                    arg_info["buffer"] = True
                    arg_info["length"] = p_b

                    l_info = real_utipia_api.get(l, {})
                    f_info = l_info.get(f_clean, {})
                    f_par = f_info.get(p_a, {})
                    f_par.update(arg_info)
                    f_info[p_a] = f_par
                    l_info[f_clean] = f_info

                    real_utipia_api[l] = l_info

            for f, p_b in uar["AllocSize"].items():
                f_clean = f.split("(")[0]
                # print(f_clean)
                if (f_clean in all_exposed_func[l] and 
                    "[" not in f):
            ...
            for f, p_b in uar["FilePath"].items():
                f_clean = f.split("(")[0]
                # print(f_clean)
                if (f_clean in all_exposed_func[l] and 
                    "[" not in f):
            ...
            if l == "libaom":
                print("get_utopia_data -- array")
                from IPython import embed; embed(); exit(1)


    return real_utipia_api
...
def get_exposed_func(targets_data):
    exposed_func = {}
    for l, cond in targets_data.items():
        exposed_func[l] = set()

        for a in cond.api_list_all:
            exposed_func[l].add(a.function_name)
...
def _main():
    ...
    all_exposed_func = get_exposed_func(targets_data)
    
    gt_data = parse_gt(groundtruth)
    utopia_data = get_utopia_data(utopia, all_exposed_func)
    ...
    for t, u_cond in utopia_data.items():
        print(f"doing {t}")
        fp_l = set()
        fn_l = set()
        stats[t] = {"source_tp": 0, "source_fp": 0, "source_tn": 0, "source_fn": 0, "init_tp": 0, "init_fp": 0, "init_tn": 0, "init_fn": 0, "sink_tp": 0, "sink_fp": 0, "sink_tn": 0, "sink_fn": 0, "varlen_tp": 0, "varlen_fp": 0, "malloc_tp": 0, "malloc_fp": 0, "malloc_tn": 0, "malloc_fn": 0, "file_path_tp": 0, "file_path_fp": 0, "file_path_tn": 0, "file_path_fn": 0}


        for func, a_gt in gt_data[t].items():
            u_func = u_cond.get(func, {})


            # a_cond = cond.conditions.get_function_conditions(a.function_name)
            # "varlen_tp": 0, "varlen_fp": 0, "varlen_tn": 0, "varlen_fn": 0
            for p, arg in a_gt.items():
                if p == "-1":
                    continue

                u_arg = u_func.get(p, {})

                # print("check utopia length  before")
                # from IPython import embed; embed(); exit(1)   

                if u_arg.get("length", "") == arg["length"]:
                    stats[t]["varlen_tp"] += 1
                else:
                    stats[t]["varlen_fp"] += 1

                
                # print("check utopia length after")
                # from IPython import embed; embed(); exit(1)   


                # # print("xxxaaa")
                # # from IPython import embed; embed(); exit(1)
                if u_arg.get("malloc_size", False):
                    if arg["malloc_size"]:
                        stats[t]["malloc_tp"] += 1
                    else:
                        stats[t]["malloc_fp"] += 1
    ...
    print("library;var-len;malloc;file-path")
    for l, s in stats.items():
        ...
        print(f"{l};{varlen};{filepath};{malloc}")
